...
export const SettingsView = ({ userName, onUserNameChange }: SettingsViewProps) => {
  ...
  const handleExport = async () => {
    try {
      ...
      a.href = url;
      a.download = `zevault-backup-${new Date().toISOString().split('T')[0]}.json`;
      a.click();
              ...
              {editingName ? (
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500"
                    autoFocus
