        ...
        <div className="flex h-full flex-col">
          <div className="border-b border-slate-200 p-4 dark:border-slate-700">
            <div className="flex items-center justify-between gap-2">
              <div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-700"><Wallet aria-hidden="true" className="h-6 w-6 text-white" /></span>{isCollapsed ? null : <span className="truncate text-xl font-bold text-slate-950 dark:text-white">Financier</span>}</div>
              <button type="button" onClick={onToggleCollapse} aria-label={isCollapsed ? t('app.openNavigation') : t('app.closeNavigation')} className="flex min-h-11 min-w-11 items-center justify-center rounded-lg hover:bg-slate-100 focus-visible:outline-2 focus-visible:outline-indigo-600 dark:hover:bg-slate-700">{isCollapsed ? <ChevronRight aria-hidden="true" className="h-5 w-5 text-slate-500" /> : <ChevronLeft aria-hidden="true" className="h-5 w-5 text-slate-500" />}</button>
            </div>
          </div>
